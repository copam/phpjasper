...
class JasperPHP
{
    ...
    public function process($input_file, $output_file = false, $format = array('pdf'), $parameters = array(), $db_connection = array(), $locale = 'pt_BR', $background = true, $redirect_output = true )
    {
        if(is_null($input_file) || empty($input_file))
            throw new \Exception('No input file', 1);
